...
public class RxOkhttp {
    private static Observable<Response> request(final OkHttpClient client, final Request request) {
    ...
    private static Observable<byte[]> streamBytes(OkHttpClient client, Request request) {
        return request(client, request).flatMap(new Func1<Response, Observable<byte[]>>() {
            @Override
            public Observable<byte[]> call(final Response response) {
                return Observable.create(new Observable.OnSubscribe<byte[]>() {
                    @Override
                    public void call(final Subscriber<? super byte[]> subj) {
                        final AsyncTask t = new AsyncTask() {
                            @Override
